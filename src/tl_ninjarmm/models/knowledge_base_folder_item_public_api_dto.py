...
class KnowledgeBaseFolderItemPublicApiDTO(BaseModel):
    ...
    is_folder: Optional[StrictBool] = Field(
        default=None, description="Indicates if this item is a folder", alias="isFolder"
    )
    ...
    def file_upload_status_validate_enum(cls, value):
        ...
        if value not in set(["PROCESSING", "SUCCESS", "FAILURE", "SUSPICIOUS"]):
            raise ValueError(
                "must be one of enum values ('PROCESSING', 'SUCCESS', 'FAILURE', 'SUSPICIOUS')"
            )
        return value
