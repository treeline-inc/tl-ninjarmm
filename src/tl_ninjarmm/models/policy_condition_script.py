...
class PolicyConditionScript(BaseModel):
    """
    Policy condition script
    """  # noqa: E501

    script_id: Optional[StrictInt] = Field(
        default=None, description="Policy condition script id", alias="scriptId"
    )
    run_as: Optional[StrictStr] = Field(
        default="SYSTEM", description="Policy condition script runAs", alias="runAs"
    )
    script_param: Optional[StrictStr] = Field(
        default=None,
    ...
    script_variables: Optional[
        Annotated[
            List[PolicyConditionScriptVariable], Field(min_length=0, max_length=20)
        ]
    ] = Field(
        default=None,
        description="Policy condition script variables",
        alias="scriptVariables",
    )
    ...
    def run_as_validate_enum(cls, value):
        ...
        if value not in set(
            ...
            raise ValueError(
                "must be one of enum values ('SYSTEM', 'LOGGED_ON_USER', 'LOCAL_ADMIN', 'DOMAIN_ADMIN', 'PREFERRED_CREDENTIAL_MAC', 'PREFERRED_CREDENTIAL_LINUX')"
            )
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PolicyConditionScript from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PolicyConditionScript from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "scriptVariables": [
                    PolicyConditionScriptVariable.from_dict(_item)
                    for _item in obj["scriptVariables"]
                ]
