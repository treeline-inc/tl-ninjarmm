...
class WindowsEventPolicyConditionCreateRequest(BaseModel):
    ...
    enabled: Optional[StrictBool] = Field(
        default=True, description="Policy condition enabled"
    )
    ...
    channels: Optional[
        Annotated[List[StrictInt], Field(min_length=0, max_length=20)]
    ] = Field(default=None, description="Policy condition notification channels")
    scripts: Optional[
        Annotated[List[PolicyConditionScript], Field(min_length=0, max_length=20)]
    ] = Field(default=None, description="Policy condition scripts")
    ...
    source: Annotated[str, Field(min_length=0, strict=True, max_length=255)] = Field(
        description="Event Source"
    )
    event_ids: Annotated[List[StrictInt], Field(min_length=0, max_length=20)] = Field(
        description="Event IDs", alias="eventIds"
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                if obj.get("notificationAction") is not None
                else "NONE",
                "notifyOnReset": obj.get("notifyOnReset")
                if obj.get("notifyOnReset") is not None
                else False,
                "resetThreshold": obj.get("resetThreshold")
                if obj.get("resetThreshold") is not None
                else 14400,
