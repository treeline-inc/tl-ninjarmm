...
from tl_ninjarmm.models.agreement_product_time_entry_dto import (
...
class TicketTimeEntryDTO(BaseModel):
    ...
    ticket_log_entry_id: Optional[StrictInt] = Field(
        default=None, alias="ticketLogEntryId"
    )
    ...
    agreement_origin_type: Optional[StrictStr] = Field(
        default=None, alias="agreementOriginType"
    )
    price: Optional[Union[StrictFloat, StrictInt]] = None
    time_tracked: Optional[StrictInt] = Field(default=None, alias="timeTracked")
    agreement: Optional[AgreementDTO] = None
    agreement_product: Optional[AgreementProductTimeEntryDTO] = Field(
        default=None, alias="agreementProduct"
    )
    ...
    def billing_validate_enum(cls, value):
        ...
        if value not in set(["BILLABLE", "NOT_BILLABLE", "NO_CHARGE"]):
            raise ValueError(
                "must be one of enum values ('BILLABLE', 'NOT_BILLABLE', 'NO_CHARGE')"
            )
        return value
    ...
    def billing_status_validate_enum(cls, value):
        ...
        if value not in set(["BILLED", "PENDING", "NOT_BILLABLE"]):
            raise ValueError(
                "must be one of enum values ('BILLED', 'PENDING', 'NOT_BILLABLE')"
            )
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of agreement_product
        if self.agreement_product:
            _dict["agreementProduct"] = self.agreement_product.to_dict()
        return _dict
        ...
        _obj = cls.model_validate(
            {
                ...
                "agreement": AgreementDTO.from_dict(obj["agreement"])
                if obj.get("agreement") is not None
                else None,
                "agreementProduct": AgreementProductTimeEntryDTO.from_dict(
                    obj["agreementProduct"]
                )
                if obj.get("agreementProduct") is not None
                else None,
            }
        )
        return _obj
