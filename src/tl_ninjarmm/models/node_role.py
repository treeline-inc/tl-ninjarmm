...
class NodeRole(BaseModel):
    ...
    custom: Optional[StrictBool] = Field(
        default=None, description="Is custom node role?"
    )
    chassis_type: Optional[StrictStr] = Field(
        default=None, description="Chassis Type", alias="chassisType"
    )
    created: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Date created"
    )
    ...
    def chassis_type_validate_enum(cls, value):
        ...
        if value not in set(["UNKNOWN", "DESKTOP", "LAPTOP", "MOBILE"]):
            raise ValueError(
                "must be one of enum values ('UNKNOWN', 'DESKTOP', 'LAPTOP', 'MOBILE')"
            )
        return value
