...
class TestTokenRefreshPerformance:
    ...
    def test_concurrent_api_calls_with_oauth(self, mock_config):
        ...
        with (
            ...
            # Verify all calls used rest_client (single transport)
            assert mock_rest_client.request.call_count == 10
...
class TestConcurrencyAndThreadSafety:
    ...
    def test_thread_safe_token_refresh(self, mock_config):
        """Test that token refresh is thread-safe."""
        with patch("tl_ninjarmm.api_client.OAuth2Session") as mock_oauth:
            mock_session = Mock()
            mock_session.fetch_token.return_value = {
            ...
            # Perform concurrent token refreshes
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(refresh_token_thread, i) for i in range(20)]
                [future.result() for future in as_completed(futures)]

            assert mock_session.fetch_token.call_count == 1, (
                f"fetch_token called {mock_session.fetch_token.call_count} times, expected 1"
            )

            # Verify final state
            assert client._token is not None
            assert client.configuration.access_token == "thread_safe_token"

    def test_concurrent_api_calls_thread_safety(self, mock_config):
        ...
        with (
            ...
            def api_call_thread(thread_id):
                ...
                response = client.call_api(
                    method="GET",
                    url=f"https://test.com/api/{thread_id}",
                    header_params={"Accept": "application/json"},
                )
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(api_call_thread, i) for i in range(50)]
                results = [future.result() for future in as_completed(futures)]
            ...
            assert mock_rest_client.request.call_count == 50

            # Verify no exceptions were raised
            for result in results:
                assert result["response"] is not None

    def test_token_refresh_race_condition_handling(self, mock_config):
        """Test that race conditions in token refresh are handled properly."""
        with (
            patch("tl_ninjarmm.api_client.OAuth2Session") as mock_oauth,
        ):
            mock_session = Mock()
            ...
            def refresh_token_thread(thread_id):
                """Refresh token in a separate thread."""
                client._token = None  # Force refresh
                client._refresh_token_if_needed()
                return thread_id

            # Perform rapid concurrent token refreshes
            with ThreadPoolExecutor(max_workers=20) as executor:
                futures = [executor.submit(refresh_token_thread, i) for i in range(100)]
                results = [future.result() for future in as_completed(futures)]

            # Verify all refreshes completed
            assert len(results) == 100

            # Verify token was set correctly despite race conditions
            assert client._token is not None
            assert client.configuration.access_token == "race_condition_token"

    def test_token_refresh_race_condition_handling_starting_with_expired_token(
        self, mock_config
    ):
        """Test that race conditions in token refresh are handled properly when starting with an expired token."""
        with (
            patch("tl_ninjarmm.api_client.OAuth2Session") as mock_oauth,
        ):
            mock_session = Mock()
            ...
            # Set up the mock to return the new token when fetch_token is called
            mock_session.fetch_token.return_value = new_token
            mock_oauth.return_value = mock_session

            # Create the client with the expired token
            client = ApiClient(configuration=mock_config)
            ...
            # Verify final state
            assert client._token is not None
            assert client.configuration.access_token == "new_token"
...
class TestStressTesting:
    ...
    def test_mixed_workload_performance(self, mock_config):
        """Test performance under mixed workload (API calls + token refreshes)."""
        with (
            patch("tl_ninjarmm.api_client.OAuth2Session") as mock_oauth,
            patch.object(ApiClient, "_refresh_token_if_needed"),
        ):
            mock_session = Mock()
            mock_session.fetch_token.return_value = {
                "access_token": "mixed_workload_token",
                "expires_in": 3600,
                "expires_at": time.time() + 3600,
            }
            mock_oauth.return_value = mock_session
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [
                    executor.submit(mixed_workload_thread, i) for i in range(100)
                ]
                results = [future.result() for future in as_completed(futures)]
            ...
            # Verify performance is acceptable
            # (No specific time assertion as this is a stress test)
