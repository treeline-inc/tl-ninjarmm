...
class TestOAuth2Initialization:
    ...
    def test_oauth2_session_not_initialized_without_credentials(self):
        ...
        with (
            ...
            # Verify oauth_session attribute is None
            assert client.oauth_session is None
            assert client._token is None
...
class TestTokenRefresh:
    ...
    def test_refresh_token_if_needed_returns_early_without_credentials(self):
        """Test that _refresh_token_if_needed returns early without credentials."""
        config = Configuration(host="https://test.com")  # No credentials
        client = ApiClient(configuration=config)
        ...
        # Verify no OAuth2Session was created
        assert client.oauth_session is None
        assert client._token is None
...
class TestErrorHandling:
    ...
    def test_refresh_token_if_needed_handles_missing_token_url(self, mock_config):
        ...
        with (
            ...
            client = ApiClient(configuration=mock_config)
            client.token_url = None  # Simulate missing token_url
